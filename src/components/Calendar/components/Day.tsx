...
export const Day = (props: IDayProps) => {
  ...
  const getActiveDays = () => {
    ...
    if (tomorrow.subtract(1, 'd').date() === moment().date() && day.date.day() !== 6 && day.date.day() !== 0) {
      return true;
    }
  ...
  const isWeekend = () => {
    if (day.name === 'пн' && day.date.day() !== 6 && day.date.dayOfYear() === moment().dayOfYear() + 2) {
      day.isMonday = true;

      return true;
    }

    if (day.name === 'пн' && day.date.day() !== 0 && day.date.dayOfYear() === moment().dayOfYear() + 1) {
      day.isMonday = true;
  ...
  const isFriday = () => {
    if (day.name === 'пн' && moment().day() === 5 && day.date.dayOfYear() === moment().dayOfYear() + 3) {
      day.isMonday = true;
  ...
  const checkClick = () => {
    ...
    if (day.date.day() !== 6 && day.date.day() !== 0) {
