      ...
      <form className='registration__form form' data-test-id='register-form' onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className={`form__inputs-box ${stepCount !== 1 ? 'hide-box' : ''}`}>
          ...
          <p className={`form__input-info ${inputError.isHighlighting ? 'highlight-error' : ''}`} data-test-id='hint'>
            {inputError.message || (
              <React.Fragment>
                ...
                <span className={`${inputError.letters ? 'highlight-error' : ''}`}>
                  &nbsp; латинский алфавит &nbsp;
                </span>
                и<span className={`${inputError.numbers ? 'highlight-error' : ''}`}>&nbsp;цифры</span>
              </React.Fragment>
            )}
          </p>
          ...
          <label className='form__label'>
            ...
            />
            <i
              className={`password-eye eye-open ${showPassword ? '' : 'hide-eye'}`}
              onClick={toggleEye}
              data-test-id='eye-opened'
            />
            <i
              className={`password-eye eye-close ${showPassword ? 'hide-eye' : ''}`}
              onClick={toggleEye}
              data-test-id='eye-closed'
            />
            {passwordError.ok && (
              <i className={`password-ok ${passwordError.ok ? 'ok' : ''}`} data-test-id='checkmark' />
            )}
          </label>

          <p
            className={`form__input-info ${passwordError.isHighlighting ? 'highlight-error' : ''}`}
            data-test-id='hint'
          >
            {passwordError.message || (
              <React.Fragment>
                Пароль
                <span className={`${passwordError.length ? 'highlight-error' : ''}`}>&nbsp;не менее 8 символов</span>, с
                <span className={`${passwordError.letters ? 'highlight-error' : ''}`}>
                  &nbsp;заглавной буквой&nbsp;
                </span>
                и<span className={`${passwordError.numbers ? 'highlight-error' : ''}`}>&nbsp;цифрой</span>
              </React.Fragment>
            )}
          </p>
          <button type='button' className='form-btn' onClick={CheckInputs} disabled={disabled}>
            следующий шаг
          </button>
        </div>
